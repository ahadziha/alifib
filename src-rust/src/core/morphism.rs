...
/// A cellular morphism: maps tags (cells) to diagrams.
#[derive(Debug, Clone)]
pub struct Morphism {
...
impl Morphism {
    /// Create an empty morphism.
    pub fn init() -> Checked<Morphism> {
        Ok(Morphism {
            table: HashMap::new(),
            by_dim: BTreeMap::new(),
            cellular: true,
    ...
    /// Build a morphism from a list of (tag, dim, cell_data, image) entries.
    pub fn of_entries(entries: Vec<(Tag, usize, CellData, Diagram)>, cellular: bool) -> Morphism {
    ...
    /// Apply morphism f to a diagram by following its paste tree structure.
    pub fn apply(f: &Morphism, diagram: &Diagram) -> Checked<Diagram> {
        ...
        if f.cellular {
            ...
            // Full reconstruction via paste tree
            apply_tree(f, &root_tree)
        }
    }

    /// Compose morphisms: g after f (g ∘ f).
    pub fn compose(g: &Morphism, f: &Morphism) -> Morphism {
        ...
        for (dim, tags) in f.domain_by_dim() {
            for tag in tags {
                ...
                let image_gf = match Morphism::apply(g, &f_entry.image) {
                    Ok(d) => d,
                    Err(_) => continue,
                };
...
fn find_undefined<'a>(f: &Morphism, tree: &'a PasteTree) -> Option<&'a Tag> {
    match tree {
        PasteTree::Leaf(tag) => {
...
fn apply_tree(f: &Morphism, tree: &PasteTree) -> Checked<Diagram> {
    match tree {
        PasteTree::Leaf(tag) => {
            f.image(tag).cloned()
        }
