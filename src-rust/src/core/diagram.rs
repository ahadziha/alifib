...
impl Diagram {
    ...
    pub fn boundary(sign: Sign, k: usize, d: &Diagram) -> Checked<Diagram> {
    ...
    pub fn boundary_normal(sign: Sign, k: usize, d: &Diagram) -> Checked<Diagram> {
    ...
    pub fn parallelism(u: &Diagram, v: &Diagram) -> Checked<(Ogposet, Embedding, Embedding)> {
    ...
    pub fn paste(k: usize, u: &Diagram, v: &Diagram) -> Checked<Diagram> {
        ...
        // Fill from v via inr (may overwrite shared boundary cells, but they agree)
    ...
    pub fn cell(tag: Tag, data: &CellData) -> Checked<Diagram> {
        match data {
    ...
    fn cell0(tag: Tag) -> Checked<Diagram> {
    ...
    fn cell_n(tag: Tag, u: &Diagram, v: &Diagram) -> Checked<Diagram> {
        ...
        // Build the new ogposet for the (d+1)-cell
        let mut faces_in: Vec<Vec<crate::core::ogposet::IntSet>> = Vec::new();
        let mut faces_out: Vec<Vec<crate::core::ogposet::IntSet>> = Vec::new();
        let mut cofaces_in: Vec<Vec<crate::core::ogposet::IntSet>> = Vec::new();
        let mut cofaces_out: Vec<Vec<crate::core::ogposet::IntSet>> = Vec::new();
        ...
        // Build trees
        // For dim < d: u's trees
        // For dim = d: Input from u, Output from v
        // For dim = d+1: Leaf(tag)
