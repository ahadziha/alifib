use alifib::interpreter::{interpret_program, Context};
...
fn main() {
    ...
    // Derive module name from file basename (strip path and .ari extension)
    ...
    let context = Context::new_empty(module_name.clone());
    let result = interpret_program(context, &program);
