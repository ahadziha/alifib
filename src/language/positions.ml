module Source = struct
  ...
  let of_path = Id.Module.of_path

  let virtual_ name =
    let display = Printf.sprintf "<%s>" name in
    Id.Module.of_path display

  let unknown = Id.Module.of_path "<unknown>"
  let to_string = Id.Module.to_string
  let equal = Id.Module.equal
  let compare = Id.Module.compare
  let hash = Id.Module.hash
  let pp = Id.Module.pp
end
...
let validate_point ~offset ~line ~column ~bol_offset =
  if offset < 0 then invalid_arg "Positions.make_point: negative offset"
    ...
    invalid_arg "Positions.make_point: line must be positive"
  else if column <= 0 then
    invalid_arg "Positions.make_point: column must be positive"
  else if bol_offset < 0 then
    invalid_arg "Positions.make_point: negative bol_offset"
...
let unknown_point =
  make_point ~source:Source.unknown ~offset:0 ~line:1 ~column:1 ~bol_offset:0
...
let pp_point fmt { source; line; column; offset; _ } =
  Format.fprintf fmt "%s:%d:%d (offset %d)" (Source.to_string source) line
    column offset

let advance start text =
  let len = String.length text in
  let rec loop idx line column bol_offset offset =
    if idx = len then { start with line; column; bol_offset; offset }
    else
      let ch = text.[idx] in
      let offset = offset + 1 in
      if Char.equal ch '\n' then loop (idx + 1) (line + 1) 1 offset offset
      else loop (idx + 1) line (column + 1) bol_offset offset
  in
  loop 0 start.line start.column start.bol_offset start.offset

let with_source point source =
  if Source.equal point.source source then point else { point with source }
...
let normalize_span start stop =
  if compare_point start stop <= 0 then (start, stop) else (stop, start)
...
let merge a b =
  ...
  ; let start =
      if compare_point a.start b.start <= 0 then a.start else b.start
    in
...
let pp_span fmt span =
  if is_point span then Format.fprintf fmt "%a" pp_point span.start
  else Format.fprintf fmt "%a-%a" pp_point span.start pp_point span.stop

let point_of_lexing source position =
  let column = position.Lexing.pos_cnum - position.Lexing.pos_bol + 1 in
  make_point ~source ~offset:position.Lexing.pos_cnum
    ~line:position.Lexing.pos_lnum ~column ~bol_offset:position.Lexing.pos_bol
...
let to_error_span span =
  Error.Located.Span.make (to_lexing span.start) (to_lexing span.stop)

let of_error_span located_span =
  let open Error.Located.Span in
  let { start; stop } = located_span in
  if not (String.equal start.Lexing.pos_fname stop.Lexing.pos_fname) then
    invalid_arg "Positions.of_error_span: mismatched span sources"
  else
    let source = Source.of_path start.Lexing.pos_fname in
    span_of_lexing source start stop
