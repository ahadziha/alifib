...
///
/// Deprecated: prefer [`RewriteEngine`] directly. This type exists only to
/// avoid breaking [`cli`](super::cli) during the migration.
...
///
/// Deprecated: prefer [`RewriteEngine::from_session`] directly.
pub fn replay_session(session: SessionFile) -> Result<SessionState, String> {
    let engine = RewriteEngine::from_session(session.clone())?;
    Ok(SessionState {
        store: Arc::clone(engine.store_arc()),
        type_complex: Arc::clone(engine.type_complex_arc()),
        source_diagram: engine.source_diagram().clone(),
        target_diagram: engine.target_diagram().cloned(),
        current_diagram: engine.current_diagram().clone(),
        running_diagram: engine.running_diagram().cloned(),
        available_rewrites: engine.available_rewrites,
        session,
    })
}
