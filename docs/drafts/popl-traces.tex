% Draft section: concurrency demonstration (interleavings vs traces).
%
...
\section{Traces, not interleavings}
...
\paragraph{Interleavings and traces.}
Consider the simplest possible concurrent system: a word $a\,b$, a rule
$r_a : a \to x$, and a rule $r_b : b \to y$. The two redexes are
disjoint; neither needs the other. An \emph{interleaving} is a run as it
is actually performed: a sequence of choices, $r_a$ then $r_b$, or $r_b$
then $r_a$---what the database literature calls a \emph{schedule} or
\emph{history} \parencite{papadimitriou_1979}, and what nearly every
model of computation simply calls an execution. The order is part of the
data even when it is meaningless. A \emph{trace}\footnote{The word is
overloaded in both directions: the traces of
\textcite{mazurkiewicz_1977} are equivalence classes of sequences, while
the traces of CSP \parencite{hoare_1985} are the sequences themselves.
The traces of this paper are diagrams, and their relation to the former
is precisely the point of this section.} is the diagram that the run
assembles: a $2$-cell
...
that could record which cell was applied first. Both interleavings build the
...
general, an interleaving is a total order on the events of a run; the trace
...
applies. Its combinatorics are exact: $2^m$ interleavings (each segment goes
...
counts interleavings exactly, by path-counting over the resulting acyclic
graph; and classifies runs into trace classes by walking every interleaving
...
\toprule
system & states & interleavings (exact) & traces & verdict \\
\midrule
...
At $N=4$ every one of the $119{,}328$ interleavings is walked, and they
...
eleven classes whose interleaving counts are $252, 210, 210, 120, 120, 45,
...
Walking interleavings is exponential, and past $150{,}000$ of them the
...
finite whatever the interleaving count, and a terminal state of the graph is
...
found without any pretence of walking a trillion interleavings. Safety over
...
graph cyclic, the interleaving space infinite, and the trace count a
