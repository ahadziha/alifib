...
- Attachment may also happen _along_ a partial map. This enables us to identify
  ...
  where `[...]` is this map given, `C` is the complex we are defining,
  `C'` is the complex defined so far, and the top is the inclusion of the
  domain of the partial map into the complex we are attaching.
