...
\emph{partial maps} (structure-preserving maps between complexes). The
interpreter maintains a mutable global state and threads it through the
...
eagerly. Every generator is entered into both $\mathit{gens}$ and
$\mathit{diags}$: the former records structural metadata while the latter
enables name resolution during diagram evaluation.
...
in the ambient complex). The term type makes this dispatch explicit.
...
by the previous. Local blocks stop on first error; complex bodies process
all instructions regardless of errors.
...
Unlike complex-body and local-block let-bindings, the type-level form does
\emph{not} check name uniqueness---it silently overwrites.
...
As at the type level, name uniqueness is not checked.
...
boundary, then adding one new top-dimensional cell. The resulting ogposet
...
Unlike the type-level variants, these check name uniqueness.
...
Assertions check \emph{definitional} (structural) equality. There are no
non-trivial equalities: everything interesting is a \emph{rewrite} (a
higher-dimensional cell). For example, the unit law
$\kwassert\;\mathit{id}\;{}\alpha = \alpha$ will \emph{fail}, because
$\mathit{id}\;{}\alpha$ has an additional cell in its ogposet. To
express the unit law, one declares a cell
$\lambda : \mathit{id}\;{}\alpha \to \alpha$.
...
\begin{mathpar}
  ...
  \inferrule*[lab=\textsc{Diag-Juxt}]
    {\EvalD{\State}{\Complex}{\DExpr_1}{D_1} \\
     \EvalD{\State}{\Complex}{\DExpr_2\;\cdots\;\DExpr_k}{D_R} \\\\
     j = \max(\min(\dim(D_1), \dim(D_R)) - 1,\; 0) \\
     D = D_1 \paste{j} D_R}
    {\EvalD{\State}{\Complex}{\DExpr_1\;\DExpr_2\;\cdots\;\DExpr_k}{D}}
\end{mathpar}

Juxtaposition is a left fold. The paste dimension is computed as
$\max(\min(\dim(D_1), \dim(D_R)) - 1,\; 0)$, clamped to be non-negative.
For two 1-cells this gives $j = 0$ (horizontal composition); for two
2-cells, $j = 1$ (vertical composition).
...
The right-hand side is evaluated before the left-hand side. This matches
OCaml's evaluation order and is preserved in the Rust implementation for
compatibility. The paste is defined only when $D_L$'s output $k$-boundary
...
\begin{mathpar}
  \inferrule*[lab=\textsc{Comp-AnonMap}]
    {\ResJ{\State}{\Cpx}{(\_, \Complex_T)} \\
     \EvalM{\State}{\Complex_T}{(\PMapDef, \Complex)}{F, \Complex_S}}
...
Name lookup checks diagrams first, then maps. Anonymous maps first resolve
the complex (which may trigger further interpretation), then evaluate the
map definition in that context. The source complex $\Complex_S$ for
...
complex $\Complex$. This is how \texttt{F.x} resolves \texttt{x} in
$F$'s domain. The result is the image of $D$ under $F$.
...
Map composition $\compose{F}{G}$ is computed lazily: apply $F$ to each
image in $G$'s table.
...
\begin{enumerate}
  ...
  \item \textbf{Evaluation order.} Implicit pasting evaluates left to
    right. Explicit pasting ($\paste{k}$) evaluates the right side first,
    matching OCaml's evaluation order.
  ...
  \item \textbf{Type-level let/def do not check uniqueness.}
    Complex-body and local-block variants do.
