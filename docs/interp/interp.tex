...
or module), and $\Namespace$ for the set of names (identifier strings
from the parser); we use $n$ as a metavariable ranging over
$\Namespace$. We use $\Downarrow$ for big-step
...
The underlying combinatorial objects are oriented graded posets; the
implementation-level representation is described in
\cref{sec:data:ogposet}.

An \emph{oriented graded poset} (ogposet) is a graded set
$U = U_0 \sqcup U_1 \sqcup \cdots \sqcup U_n$ together with, for each
element $x \in U_k$ with $k > 0$, two sets of faces: an
\emph{input boundary} $\bdyin{} x \subseteq U_{k-1}$ and an
\emph{output boundary} $\bdyout{} x \subseteq U_{k-1}$.
Intuitively, elements of $U_k$ are $k$-dimensional cells, input faces
are ``source'' faces, and output faces are ``target'' faces.
The \emph{$k$-boundary} $\bdy{\pm}{k}{U}$ is the sub-ogposet
consisting of all cells reachable from the $\pm$-extremal cells at
dimension~$k$ and below.
...
A molecule is \emph{round} if, at every dimension $k$ below the top, the
interiors of the input and output $k$-boundaries are disjoint:
\[
  \bdy{-}{k}{U} \cap \bdy{+}{k}{U} = \partial_{k-1}(U)
  \qquad \text{for all } k < \dim(U),
\]
where $\partial_{k-1}(U)$ denotes the full $(k{-}1)$-boundary.
All globes are round; so is any vertical paste
$U \paste{k} V$ where $k = \dim(U) - 1$.  Roundness matters because the
atom constructor \emph{requires} its source and target to be round,
ensuring that the resulting cell has well-defined globular boundaries:
$\bdy{\alpha}{k}{\bdy{\beta}{n}{U}} = \bdy{\alpha}{k}{U}$ for all
$k < n$.
...
\begin{itemize}
  \item $\cells : \GIds \rightharpoonup \CData$ maps
    global identifiers to their cell data. (Dimensions are stored in a
    separate side-table for efficient dimension-based iteration.)
  \item $\types : \GIds \rightharpoonup (\CData \times \Complex)$ maps
    global identifiers of types to their cell data and associated complex.
  \item $\modules : \ModId \rightharpoonup \Complex$ maps canonical
...
\[
  \Tag \Coloneqq \TagG{\GId} \mid \TagL{n}
\]
...
\begin{itemize}
  \item $\mathit{gens} : \Namespace \rightharpoonup
    (\Tag \times \mathbb{N} \times \Diag{D})$
    maps generator names to their tag, dimension, and
    \emph{classifier diagram} (the diagram consisting of that single cell).
  \item $\mathit{diags} : \Namespace \rightharpoonup \Diag{D}$ maps diagram names
    to diagram values (labelled ogposets; see \cref{sec:data}).
  \item $\mathit{maps} : \Namespace \rightharpoonup
    (\MapDom \times \Map{F})$ maps partial map names to a domain
    annotation and a partial map value (a tag-indexed table;
    see \cref{sec:data}).
    The domain is either $\TyDom(\GIds)$ or $\ModDom(\ModId)$.
  \item $\mathit{locals} : \Namespace \rightharpoonup \CData$
...
Before describing the evaluation rules, we define the three data
structures that the interpreter manipulates: oriented graded posets
(the combinatorial shapes introduced in \cref{sec:molecules}), diagrams
(labelled shapes with paste history), and partial maps
...
\end{itemize}
Ogposets are the unlabelled shapes underlying all diagrams.
In the Rust implementation, cells at each dimension are indexed by
position ($0, 1, \ldots$) and face/coface sets are stored as sorted
vectors of indices.
...
\begin{itemize}
  ...
  \item \textbf{Roundness.}
    ...
    disjoint at every dimension---a necessary condition for $G$ to be
    a valid diagram shape.
\end{itemize}
...
A \emph{diagram} $\Diag{D}$ consists of three components:
\begin{itemize}
  \item A \emph{shape}: an ogposet $G$ (shared via reference counting).
  \item \emph{Labels}:
    for each dimension $d$, a map
    $\ell_d : G_d \to \Tag$
    assigning a tag to every cell in the shape.
  \item \emph{Paste trees}: for each dimension $d$ and sign
    $s \in \{-,+\}$, a tree recording how the $d$-level structure
    was built by pasting.
\end{itemize}

The shape gives the combinatorial structure; the labels record
\emph{which} generators occupy each position; the paste trees record
\emph{how} the diagram was assembled.
...
A \emph{partial map} $\Map{F}$ is a finite map
$\Tag \rightharpoonup (\CData, \Diag{D})$
sending source tags to their cell data and image diagram.
The domain is indexed by dimension for ordered iteration.
A Boolean \emph{cellular} flag records whether every image is a
single cell (top-level paste tree is a $\mathsf{Leaf}$).
  ...
  \inferrule*[lab=\textsc{CAttach}]
    {\AddrJ{\State}{\Addr}{\GId_A} \\
     ...
     \text{for each } (n_g, t_g, \dim_g) \in \Complex_A.\mathit{gens}
       \text{, by ascending dimension, with } F_i(t_g)\up\text{:} \\
