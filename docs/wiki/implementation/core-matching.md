...
- `Embedding { dom, cod, map, inv }` (`embeddings.rs`) — injective ogposet map,
  ...
  `id`, `empty`, `make` constructors (`make` is unchecked).
- `RulePattern { pattern, pattern_to_rewrite, backward }` — precomputed rule side.
- `CandidateMatch { rule_name, image_positions, iso_emb }` *(internal)* — a
  confirmed isomorphism, no step yet.
- `MatchResult { step, members, image_positions }` — a confirmed rewrite; `step`
  ...
  match (singleton for an ordinary rewrite); `image_positions` is their union.
- `pushout::{Span, MultiPushout, Pushout}` — all `pub(super)`; the colimit data.

### Entry points

Production consumes four `pub(crate)` functions of `matching.rs`:

- `build_rule_patterns(complex, n, backward)` — one `RulePattern` per rule of
  dimension $n+1$, built once per session/direction.
- `for_each_rule_candidate` + `confirm_candidate` — lazy candidate stream and
  singleton confirmation. The engine's `collect_confirmed_matches` (the rewrite
  menu) and `find_first_match` (non-parallel auto) are thin loops over this pair
  (`src/interactive/engine.rs`).
- `greedy_parallel_auto_step` — the live parallel path: `RewriteEngine::auto`
  when `parallel` is set, and the interpreter's auto-normalisation
  (`src/interpreter/diagram.rs`).
- `try_family_from_members` — `RewriteEngine::step_multi`: the user picks
  disjoint matches from the menu and the combined parallel step is rebuilt from
  the already-confirmed `FamilyMember`s.

(`find_matches` is `#[cfg(test)]`-only and walks the same path for a single rule;
production never calls it.)

## Data flow of one rewrite step

```
target V ──TargetFlowData::new──▶ flow graph F(V) (built once, reused per rule)
rule (n+1)-cell ──RulePattern::new──▶ pattern U + pattern_to_rewrite
   ...
   multi_pushout(V.shape, spans)   merge labels (target ∪ rewrites)
                     │  tip ogposet + labels
                     ▼
   tip.dim ≤ 3 ? assemble_low_dim_step : reconstruct::reconstruct
                     │
                     ▼
...
`RulePattern::new(rewrite, backward)` slices the rule's classifier once per
session so long-running sessions don't redo the boundary work every step. It
records:
...
Hadzihasanovic–Kessler, *Higher-Dimensional Subdiagram Matching*, LICS 2023 —
`docs/papers/2304.09216v1.pdf`) builds a `DiGraph` whose nodes are all cells of
dimension $> k$ and whose edges record $\Delta^+_k(x) \cap \Delta^-_k(y) \neq
\varnothing$ — the output $k$-boundary of $x$ meets the input $k$-boundary of
$y$ (test `flow_graph_two_arrow_paste`). So matching $U$ inside $V$ becomes a
**labelled, path-induced subgraph isomorphism** of $\mathbf{F}(U)$ into
$\mathbf{F}(V)$.

`TargetFlowData::new(target)` builds $\mathbf{F}(V)$ once; `for_each_rule_candidate`
then drives `for_each_candidate_in_rule` over every rule of the right dimension.
That inner loop rebuilds the (small) pattern flow graph and calls
`find_path_induced_matches`: backtracking with label-filtered candidate sets,
most-constrained-variable ordering, and the path-induced constraint (edge in
pattern iff edge in target image). Each surviving vertex match yields a tentative
`image_positions` (the top-cells it covers); covers already seen are deduplicated,
covers meeting the caller's `occupied` set are skipped *before* the expensive
isomorphism check. dim-0 targets are special-cased in `for_each_candidate_dim0`
(label-equality scan over points).
...
image in the base; un-shared cells become fresh — appended after the base cells —
and their faces/cofaces are remapped through the running `inr_map`.

### 5. Recover a diagram — `reconstruct`

`multi_pushout` yields an ogposet and labels, but a [[diagram]] also carries a
**paste history** (`BoundaryHistory` / `PasteTree`). Two paths, switched on
`tip.dim` — the *step's* dimension $n+1$:

- **tip dim ≤ 3** (`assemble_low_dim_step`): the candidate paste tree is provably
  ...
  its output (or input, if backward) boundary tree.
- **tip dim > 3** (`reconstruct::reconstruct`): build a paste tree, `realise_tree`
  it, and `check_sizes` against the pre-diagram.

`reconstruct` itself is a recursive layering algorithm (base case: a single
maximal element, `layering_dimension() == -1`, becomes a `PasteTree::Leaf`). It
picks a *decomposition dimension* $k$ — for $\dim \le 3$ the frame dimension,
i.e. the greatest $k$ whose maximal flow graph has an edge (falling back to 0);
for $\dim > 3$ the layering dimension — builds $\mathbf{M}_k$
...
$\dim \le 3$ a single topological sort suffices; for $\dim > 3$ it tries sorts
lazily until one realises correctly (`graph::try_topological_sorts`, capped at
10 000). Tests `reconstruct_all_category` and `reconstruct_all_semigroup` pin
deep attach hierarchies; `idem_chain_reaches_rhs` pins the end-to-end step.
...
`occupied` set; `try_or_shrink` then verifies the family, and on failure pops
the last member, greedily *re-extends* the prefix with fresh disjoint matches,
retries, and finally recurses on the bare prefix. Evidence:
`greedy_parallel_in_four_chain` (greedy finds the size-2 family in a four-chain
and its step's output boundary is correct).
...
one-element slice; `try_family_from_members` is the same construction from
pre-confirmed members).
...
- **One candidate per top-cell cover.** `for_each_candidate_in_rule` deduplicates
  vertex matches by `image_positions`: only the first flow match covering a given
  set of top cells is ever confirmed.
- **The `occupied` prefilter is the greedy speedup.** Candidates overlapping the
  occupied set are dropped before their isomorphism check — that, not the family
  search, is what keeps `build_greedy_family` passes cheap.
- **Backward rewriting** swaps which boundary is the pattern (`Sign::Output`
  ...
  path. `assemble_low_dim_step` reads `backward` off the *first* family member
  only — sound because `build_rule_patterns` builds the whole pattern map for one
  direction, so families are direction-homogeneous by construction.
- **Tip dim ≤ 3 trusts the tree.** `assemble_low_dim_step` deliberately skips
  `realise_tree` + `check_sizes` (the condition is on the *step's* dimension,
  `mp.tip.dim`). If that fast path ever produces a malformed diagram, suspect
  this shortcut first.
...
a [[molecule]]/[[diagram]]. The [[pushout]] glues (pastes) the rule's cell onto
the target — it builds a *larger* diagram, not a composite reduced to one cell.
The [[flow-graph]] $\mathbf{F}_k$ and the layering used in [[reconstruction]]
come from Hadzihasanovic–Kessler, LICS 2023 (Definition 61). See [[core-diagram]] for
`Diagram`/`BoundaryHistory`, [[core-paste-tree]] for `PasteTree`/`realise_tree`,
[[core-complex]] for `classifier` and generator lookup, and
[[interactive-engine]] for how steps are driven in a session.
