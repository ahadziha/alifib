...
The CLI REPL is a *thin adapter*: `repl` reads a line, parses it with the shared
`command` parser, hands the resulting `Request` to `Session::apply`
([[interactive-session]]), turns the `ResponseData` into a `RichText` with
`richtext`, and styles it to the terminal with `display`. Command semantics and
canonical messages live in [[interactive-session]]; the *layout* lives in
`richtext`, shared verbatim with the web. This page covers the CLI-local pieces —
the readline loop, argv parsing, the structured renderer, and the ANSI styler.

## What it owns

Five files, but only two of them are CLI-specific. `repl.rs` (gated behind
`#[cfg(feature = "cli")]` in `mod.rs`) is the read–eval loop; `cli.rs` parses the
argv that launches it and the other interactive subcommands. The remaining three
are **shared with the web**: `richtext.rs` is the single producer of display
layout, `display.rs` is the ANSI styler that renders `RichText` to a terminal,
and `render.rs` builds the bracketed match-display string. The session state
machine they drive is [[interactive-session]]; the rewrite arithmetic is
[[interactive-engine]].
...
| `Display` (`new` / `plain` / `style`) | `display.rs` | terminal sink; the **only** place ANSI codes live |
...
semicolon-separated part is parsed and dispatched in turn, so `start C a b ;
apply 0 ; show` runs as three commands. `run_repl` keeps a single piece of
mutable state across iterations — the `Session` — plus the `rustyline` editor;
everything else (the store, source, backward flag, active engine/fill) lives
inside the `Session`.
...
`session.store()` via `build_types_from_store` / `build_type_detail_from_store` /
`build_homology_data`, no session needed) — and routes **everything else**
through `to_request` → `Session::apply`. `start`/`resume` are built here because
they also need the session's `root_path`; the rest is the shared
`Command::to_request`.
...
`Proof`, `Store`, `Holes`, `Types`, `TypeDetail`, `Homology`; `None` for the
message-only replies like `stop`/`done`/`save`/`backward`/`parallel`), and
`render_response(kind, data)` produces the lines: the active-rewrite state with
its `[bracketed]` matches (`match_segments` re-parses `match_display` into
`Redex`/`Label` runs), a 0-cell fill's candidate list, the open-holes and
constraints listing, type detail, homology, and so on. `help(web)` builds the
command-list `RichText`, dropping the CLI-only rows (`print`/`save`/`quit`) for
the web and adding the web-only `clear`. Layout and wording live here once; the
CLI and web differ only in the role→style table. **Plain-mode invariant**:
concatenating every segment's `text` (with `Redex` wrapped in `[…]`) reproduces
the uncoloured transcript exactly.
...
gets ANSI; `Display::plain()` forces colour off for tests and deterministic
rendering. `Display::style(&RichText)` maps each `Segment`'s `Role` to an ANSI
code (the 16-colour palette mirrors the web frontend's CSS roles), or to plain
text (with `[…]` brackets for `Redex`) when colour is off — this is the **only**
place in the codebase ANSI codes appear. The thin message methods
`meta`/`error`/`file`/`inspect_rich` print a one-line reply or an already-styled
block; the inline painter `acc` colours the `❯` prompt.
...
chaining paste nodes as `(… #k …)`, matching the house $\#_k$ notation. Its one
caller is `protocol::build_rewrite_info_from_family` *(internal)*, which stores
the string in `RewriteInfo.match_display` ([[interactive-daemon-web]]);
`richtext::match_segments` then re-segments it for display. It degrades to `"?"`
on a step with no input tree rather than panicking.
...
`--emacs`. `run_repl_cmd` and `run_serve_cmd` dispatch from here (`run_serve_cmd`
requires all of `<file>`/`--type`/`--source` or none); `web`/`mcp` parse their
args here but launch elsewhere ([[web-backends]]).
...
- **`Display::new` decides colour once**, at construction from `is_terminal()`;
  ...
  `cli_transcript_matches_web_style` (tests/cli_render.rs) pins the plain-mode
  transcript verbatim.
- **The renderer is medium-neutral; only `display` knows ANSI.** Changing a
  colour is one edit in `display.rs`; changing *what is shown* is one edit in
  `richtext.rs`, and it lands on the web at the same time —
  `cli_and_web_responses_are_identical` drives the same script through `WebRepl`
  and asserts both `data` and the `rendered` segments are byte-identical.
- **One parser, both front-ends.** `command::parse(line, Frontend)` is shared
  with the web; `errors_are_identical_strings` and
  `medium_specific_commands_are_gated` (src/interactive/command.rs) pin that the
  wording is identical and that `print`/`save`/`quit` vs `clear` are gated per
  medium.
...
- **`to_request` builds `start`/`resume` itself.** Those carry the source path
  the shared `Command::to_request` does not have; for every other command it
  defers to the shared mapping, seeded with the idle `backward` mode.
