...
backend feeds to `Request` / `engine.handle`. Session birth and death are *not*
wire commands here — they go through the dedicated
`start_session`/`resume_session`/`stop_session`/`reset` methods, mirroring the
`WebRepl` state machine. See [[web-backends]] for the three backend crates and
